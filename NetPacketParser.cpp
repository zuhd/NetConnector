...
uchar key1[16] = { 117, 42, 81, 222, 211, 85, 189, 17, 
92, 159, 6, 102, 179, 2, 100, 0 };

uchar key2[16] = { 127, 31, 79, 202, 251, 81, 179, 67, 
...
{	
	for(int i = 0; i < 16; ++i)
	{
		m_szSymmKey[i] = (key1[i] + 'a') ^ (key2[i] + 'v');
	}
	//m_shSeed = 1606;
	m_shSeed = 1128;
}
...
int CNetPacketParser::EncodePacket(const char* pInData, int nInLen, char* pOutData, int& nOutLen)
{
	...
	//压缩数据包
	unsigned char* pData = (unsigned char*)(pOutData + 6);

	if(nInLen > 512)
	{
		int ret = compress((Bytef*)pData, (uIntf*)&nPacketLen, (Bytef*)pInData, nInLen);
		if(ret != Z_OK)
		{
	...
	else
	{
		...
		{
			cCRC += (pData[i] << 2);
			pData[i] = (i % 3 == 1) ? ((pData[i] << 2) | (pData[i] >> 6))
				: ((pData[i] << 5) | (pData[i] >> 3));
		}
		cCRC ^= m_szSymmKey[12];
...
int CNetPacketParser::DecodePacket(const char* pInData, int nInLen, char* pOutData, int& nMaxLen)
{
	if(nInLen < 6)
		return 0;
	char cCompress = 0;
	...
	else
	{
		if(cCompress != 1)
		{
			...
			unsigned char* pData = (unsigned char*)pOutData;
			...
			for(int i = 0; i < usPacketLen; ++i)
			{
				pData[i] = (i % 3 == 1) ? ((pData[i] << 6) | (pData[i] >> 2)) :
			((pData[i] << 3) | (pData[i] >> 5));
			cValiate += (pData[i] << 2);
			}
			cValiate ^= m_szSymmKey[12];
			if(cCRC != cValiate)
			{
				nMaxLen = 0; //数据包错误
				return 6; //4个字节作废
		...
		else
		{
			...
			//解压数据
			int ret = uncompress((Bytef*)pOutData, (uIntf*)&nMaxLen, (Bytef*)pData, usPacketLen);
			if(ret != Z_OK)
			{
				glog.log("zlib compress failed ret %d", ret);
				return 6;
